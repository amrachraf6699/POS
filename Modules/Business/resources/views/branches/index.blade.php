...
@section('navigation')
    <div class="flex w-full items-center justify-between">
        ...
        @if($canManage)<a href="{{ route('business.branches.create') }}" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-bold text-white">إضافة فرع</a>@endif
    </div>
@endsection
    ...
    <div class="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
        @forelse ($branches as $branch)
            <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                @if($canManage)<a href="{{ route('business.branches.edit', $branch) }}" class="mt-5 inline-block text-sm font-bold text-indigo-600">إدارة الفرع ←</a>@endif
            </article>
        @empty
