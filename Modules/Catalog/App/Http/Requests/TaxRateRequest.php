...
final class TaxRateRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return ['name' => ['required', 'string', 'max:255'], 'rate_basis_points' => ['required', 'integer', 'between:0,10000'], 'effective_from' => ['required', 'date', Rule::unique('tax_rates', 'effective_from')->where(fn ($query) => $query->where('tenant_id', app(TenantContext::class)->id())->where('name', $name))->ignore($ignore)]];
    }
}
