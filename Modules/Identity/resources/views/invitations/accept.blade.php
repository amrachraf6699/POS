...
@section('content')
    <div class="mx-auto max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <form method="POST"
            action="{{ URL::temporarySignedRoute('invitations.accept.store', $invitation->expires_at, ['invitation' => $invitation, 'token' => $token]) }}"
            class="mt-6 space-y-5">
            @csrf
            <label class="block text-sm font-medium text-slate-700">الاسم
                <input name="name" value="{{ old('name') }}" required
                    class="mt-2 w-full rounded-xl border-slate-300 px-4 py-3">
            </label>
