...
class BusinessDashboardTest extends TenantIsolationTestCase
{
    public function test_active_members_can_open_dashboard_and_owner_sees_management_metrics(): void
    {
        ...
        $response->assertOk()
            ->assertSee('مرحباً')
            ->assertSee('إعدادات النشاط')
            ->assertSee('إدارة الفروع')
            ->assertSee('دعوة أعضاء')
            ->assertSee('قريباً');
    }
    ...
    public function test_manager_and_ordinary_members_see_only_their_accessible_branch_data(): void
    {
        ...
        $memberMembership = Membership::factory()->create([
            'tenant_id' => $tenant->getKey(), 'user_id' => $member->getKey(), 'role' => 'staff',
        ]);
        ...
        BranchAssignment::factory()->create([
            'branch_id' => $assigned->getKey(), 'user_id' => $manager->getKey(), 'tenant_id' => $tenant->getKey(),
        ]);
        ...
        $managerResponse->assertOk()->assertSee('إعدادات النشاط')->assertSee('إدارة الفروع')->assertSee('دعوة أعضاء');
        ...
        $memberResponse->assertOk()->assertDontSee('إعدادات النشاط')->assertDontSee('دعوة أعضاء')->assertSee('الفروع المتاحة');

        $this->actingAs($member)->withSession(['current_tenant_id' => $tenant->getKey()])
            ->get('/tenant/branches')->assertOk()->assertDontSee('إضافة فرع');
    ...
    public function test_home_is_a_compatibility_redirect_to_the_tenant_dashboard(): void
    {
        ...
        $this->actingAs($owner)->withSession(['current_tenant_id' => $tenant->getKey()])
            ->get('/home')->assertRedirect(route('business.dashboard'));
    }
