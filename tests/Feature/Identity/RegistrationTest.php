...
class RegistrationTest extends TestCase
{
    ...
    {
        $this->get('/register')->assertOk()->assertSee('lang="ar"', false)->assertSee('dir="rtl"', false)->assertSee('ابدأ بإدارة نشاطك اليوم');
    }

    public function test_registration_creates_and_authenticates_an_owner_atomically(): void
    {
        $this->get('/register')->assertOk();
        $response = $this->post('/register', ['_token' => csrf_token(), 'name' => 'Amr Achraf', 'email' => 'owner@example.com', 'password' => 'password-123', 'password_confirmation' => 'password-123', 'tenant_name' => 'Cairo Store']);
        $response->assertRedirect('/home');
        $this->assertAuthenticated();
        $this->assertSame(Tenant::query()->where('slug', 'cairo-store')->value('id'), session('current_tenant_id'));
    ...
    public function test_generated_slug_collision_is_suffixed_deterministically(): void
    {
        ...
        $result = app(RegisterOwnerAction::class)->execute(new RegisterOwnerData(name: 'Second Owner', email: 'second@example.com', password: 'password-123', tenantName: 'Cairo Store'));
    ...
    public function test_duplicate_explicit_slug_rolls_back_registration(): void
    {
        ...
        try {
            app(RegisterOwnerAction::class)->execute(new RegisterOwnerData(name: 'Owner', email: 'owner@example.com', password: 'password-123', tenantName: 'New Store', tenantSlug: 'fixed-store'));
    ...
    public function test_inactive_users_memberships_suspended_tenants_and_deleted_tenants_are_inaccessible(): void
    {
        ...
        /** @var Tenant $activeTenant */ $activeTenant = Tenant::factory()->create();
        /** @var Membership $membership */ $membership = Membership::factory()->create(['user_id' => $user->id, 'tenant_id' => $activeTenant->id]);
